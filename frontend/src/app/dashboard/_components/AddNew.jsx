...
function AddNew() {
    ...
    const onSubmit = async (e) => {
        ...
        if (JsonResp) {
            try {
                // Call our API instead of Prisma directly
                const res = await fetch("/api/interview", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        interview_id: uuidv4(),
                        candidate_id: 1, // later replace with logged-in user ID
                        job_position: jobPos,
                        job_description: jobDesc,
                        experience_level: parseInt(jobExp),
                        feedback: JsonResp,
                    }),
                })
                ...
                if (res.ok) {
                    console.log("Inserted Interview:", newInterview)
                    setOpenDialog(false)
                    router.push(`/dashboard/interview/${newInterview.interview_id}`)
                } else {
                    console.error("API error:", newInterview.error)
                }
            } catch (err) {
                console.error("Network/DB error:", err)
            }
        } else {
            console.log("ERR – No JSON response")
        }
